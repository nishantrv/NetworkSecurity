...
class DataValidation:
    ...
    def validate_if_numerical(self, dataframe: pd.DataFrame) -> bool:
        try:
            if "numerical_columns" in self.schema_config:
                columns_numerical = self.schema_config["numerical_columns"]
            else:
                columns_numerical = []
    ...
    def initiate_data_validation(self)->DataValidationArtifact:
        try:
            ...
            status = self.validate_if_numerical(dataframe=train_dataframe)
            if not status:
                error_message = f"Train dataframe has insufficient numerical columns"

            status = self.validate_if_numerical(dataframe=test_dataframe)
            if not status:
                error_message = f"Test dataframe doesn't contain all columns"
